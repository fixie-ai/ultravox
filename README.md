...
2. You want to improve the knowledge of the model --> NO NEED TO TRAIN ULTRAVOX!
    ...
    a. We suggest to either use RAG on the fly (no training needed), or fine-tune the LLM backbone instead. If you fine-tune the LLM, you might need to re-train Ultravox (see previous step) unless the embedding layer stays frozen.
