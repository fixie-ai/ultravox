...
class History:
    ...
    def add_message(self, message: Dict[str, str], audio_token_len: int):
        ...
        if audio_token_len > 0:
            if content.count(self.audio_placeholder) != 1:
                raise ValueError(f"Expected 1 audio placeholder, found {content.count(self.audio_placeholder)}")
            message['content'] = content.replace(self.audio_placeholder, self.audio_token_replacement * audio_token_len)
    ...
    def update_key_values(self, key_values: Union[Tuple, transformers.cache_utils.Cache]):
