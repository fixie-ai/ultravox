...
class TestModifiedWhisperEncoder:
    @pytest.fixture
    def encoder(self):
    ...
    def test_init_latency_mask_none(self, encoder):
        encoder.init_latency_mask(None, torch.float32)
        assert encoder.audio_streaming_mask is None

    def test_init_latency_mask_valid(self, encoder):
        block_size = 50
        ...
        # 50x60=3000
        source_mask = torch.tril(torch.ones(60, 60), diagonal=0).repeat_interleave(block_size, dim=0).repeat_interleave(block_size, dim=1)
        source_mask = (1.0 - source_mask) * torch.finfo(torch.float32).min
        assert torch.allclose(mask, source_mask)

    def test_init_latency_mask_invalid_block_size(self, encoder):
