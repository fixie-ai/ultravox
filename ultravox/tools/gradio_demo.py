...
def main():
    ...
    def process_turn(chatbot: gr.Chatbot, prompt: str, audio: Optional[Tuple[int, np.ndarray]] = None, num_beams: int = 1, temperature: float = 0):
        ...
        if len(sample.messages) != 1:
            raise ValueError(f"Expected exactly 1 message in sample but got {len(sample.messages)}")
    ...
    with gr.Blocks() as demo:
        ...
        prompt.submit(add_text, [chatbot, prompt], [chatbot], queue=False
        ).then(
            process_text, [chatbot, prompt, num_beams, temperature], [chatbot, prompt], queue=False
        )
        audio.stop_recording(add_audio, [chatbot, audio], [chatbot], queue=False
        ).then(
