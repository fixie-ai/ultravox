...
class CustomWavTokenizer(torch.nn.Module):
    def __init__(self, config_path, model_path, sample_rate=16000, vocab_size=4096):
        ...
        self.wavtokenizer: WavTokenizer = WavTokenizer.from_pretrained0802(
    ...
    def _tokenize(self, wav):
        ...
        _, discrete_code = self.wavtokenizer.encode_infer(
    ...
    def save_pretrained(self, save_directory):
        ...
        with open(
            os.path.join(save_directory, self.vocab_files_names["config_file"]), "w"
        ) as f:
            json.dump(config, f)

        # Save the model weights
        torch.save(
            self.wavtokenizer.state_dict(),
            os.path.join(save_directory, self.vocab_files_names["model_file"]),
        )

        # Save vocabulary
        with open(
            os.path.join(save_directory, self.vocab_files_names["vocab_file"]), "w"
        ) as f:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *init_inputs, **kwargs):
        ...
        config_file = os.path.join(
            pretrained_model_name_or_path, cls.vocab_files_names["config_file"]
        )
        ...
        model_file = os.path.join(
            pretrained_model_name_or_path, cls.vocab_files_names["model_file"]
        )
        tokenizer.wavtokenizer.load_state_dict(
            torch.load(model_file, map_location=tokenizer.device)
        )

        # Load vocabulary
        vocab_file = os.path.join(
            pretrained_model_name_or_path, cls.vocab_files_names["vocab_file"]
        )
